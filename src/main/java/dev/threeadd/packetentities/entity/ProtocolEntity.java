...
import dev.threeadd.packetentities.entity.status.EntityStatus;
...
public class ProtocolEntity {
    ...
    private final ProtocolObjectData objectData;
    private ProtocolEntityMeta entityMeta;
    ...
    ProtocolEntity(ProtocolEntityBuilder builder) {
        ...
        // base first so managers can access the data
        ...
        this.objectData = builder.objectData != null ? builder.objectData : ProtocolObjectData.EMPTY;
        this.entityMeta = builder.entityMeta != null ? builder.entityMeta : new ProtocolEntityMeta(this.entityType);
        this.dataVersion = builder.dataVersion != null ? builder.dataVersion : PacketEvents.getAPI().getServerManager().getVersion();
        ...
        this.attributesStateManager = new AttributesStateManager(this, builder.properties != null ? builder.properties : Collections.emptyList());

        if (!isSubtypeOf(this.objectData.getBoundEntityType())) {
        ...
        if (!isSubtypeOf(this.entityMeta.getSchema().getBoundEntityType())) {
            throw new IllegalArgumentException("Entity type " + this.entityType +
                    " is not compatible with metadata schema bound entity type " + this.entityMeta.getSchema().getBoundEntityType());
        }

        // attach after all data is set
    ...
    public EventBus getEventBus() {
        return this.extensionManager.getEventBus();
    }
     ...
     */
    public @UnmodifiableView Collection<Class<? extends EntityExtension>> getExtensionClasses() {
     ...
     */
    public <T extends Event> RegisteredViewerRule addViewersUpdateTrigger(EventBus eventBus, Class<T> eventClass, Function<T, Collection<User>> userExtractor) {
        return this.viewerManager.addViewersUpdateTrigger(eventBus, eventClass, userExtractor);
    }

    /**
     * Sends a packet to all active viewers of this entity.
     *
     * @param packets the packets to send
     * @return this protocol entity for chaining
     */
    public ProtocolEntity sendPacketsToViewers(@Nullable PacketWrapper<?>... packets) {
    ...
    /**
     * Checks if this entity type is equal to or a subtype of the specified entity type.
     *
     * @param other the {@link EntityType} to check against
     * @return {@code true} if this entity type is the same as, or a descendant of,
     * the specified {@code other} entity type; {@code false} otherwise
     */
    public boolean isSubtypeOf(EntityType other) {
        return (this.entityType.isInstanceOf(other));
    }
     ...
     * To update for viewers you can either get and set using {@link #setEntityMeta(ProtocolEntityMeta)}
     * or alternatively you can use {@link #editMeta(Consumer)}.
     *
     * @return this entity's meta
     */
    public ProtocolEntityMeta getEntityMeta() {
     ...
     *
     * @param entityMeta the new meta
     * @return this entity for chaining
     */
    public ProtocolEntity setEntityMeta(ProtocolEntityMeta entityMeta) {
        this.entityMeta = entityMeta;

        if (!this.isSpawned) return this;
        sendPacketsToViewers(this.entityMeta.createPacket(this.entityId, this.dataVersion));
        return this;
    }
     ...
     * After this you will have to implement updating using {@link #addViewersUpdateTrigger(EventBus, Class, Function)} or manually using {@link #updateViewer(User)}.
     *
    ...
    public ProtocolEntity spawn() {
        ...
        this.extensionManager.getEventBus().post(event);
    ...
    public void destroy() {
        ...
        PacketEntities.getAPI().getEntityManager().removeEntity(this);
    }
    ...
    /**
     * Sends a {@link WrapperPlayServerEntityStatus} to viewers
     *
     * @param status the {@link EntityStatus} to include in the packet
     * @return this entity for chaining
     * @throws IllegalArgumentException if this entity's {@link #getEntityType()} isn't a subtype of the passed status'
     *                                  {@link EntityStatus#boundType()}, see {@link #isSubtypeOf(EntityType)}
     */
    public ProtocolEntity playStatus(EntityStatus status) {
        if (!isSubtypeOf(status.boundType()))
            throw new IllegalArgumentException("The status provided is not a sub type of this type.");
        return sendPacketsToViewers(new WrapperPlayServerEntityStatus(this.entityId, status.index()));
    }
    ...
    public String toString() {
        return String.format(
                ...
                this.uuid,
                this.isSpawned,
                this.worldStateManager.getWorldState(),
                this.viewerManager.getViewers(),
                this.extensionManager.getExtensionClasses().stream().map(Class::getSimpleName).collect(Collectors.toList()),
                this.entityMeta,
                this.attributesStateManager.getAttributesState().currentProperties()
        );
    }
