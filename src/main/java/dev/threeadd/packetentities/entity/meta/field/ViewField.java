...
/**
 * Represent a {@link EntityMetaField} where the value is a {@link EntityMetaView}
 *
 * @param <T> The type of data the view represents
 * @param <V> The view wrapper type
 * @see SimpleField
 */
public final class ViewField<T, V extends EntityMetaView<T>> extends EntityMetaField<T> {
    ...
    public static <T, V extends EntityMetaView<T>> Builder<T, V> builder(Function<ProtocolEntityMeta, V> wrapperFactory) {
        return new Builder<>(wrapperFactory);
    }

    public static class Builder<T, V extends EntityMetaView<T>> extends BaseBuilder<T, Builder<T, V>> {
