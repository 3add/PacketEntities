...
public class ViewerManager {
    ...
    public void addViewer(UUID playerUuid, boolean isChunkLoad) {
        ...
        defaultPackets.add(this.entity.getWorldStateManager().createSpawnPacket());
        /*
        ...
        if (NEWER_OR_EQUALS_1_20_2) {
            defaultPackets.add(this.entity.getEntityMeta().createPacket(this.entity.getEntityId(), this.entity.getDataVersion()));
        }
        ...
        PacketDispatcher.sendPackets(playerUuid, event.packets());
    }
    ...
    public <T extends Event> RegisteredViewerRule addViewersUpdateTrigger(EventBus bus, Class<T> eventClass, Function<T, Collection<User>> userExtractor) {
    ...
    public void sendPacketsToViewers(@Nullable PacketWrapper<?>... wrappers) {
        for (UUID uuid : this.viewers) {
            PacketDispatcher.sendPackets(uuid, wrappers);
        }
    }
