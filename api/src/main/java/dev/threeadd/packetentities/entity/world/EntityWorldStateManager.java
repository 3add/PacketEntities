...
public class EntityWorldStateManager {
    ...
    public PacketWrapper<?> createSpawnPacket() {
        EntityType type = this.entity.getEntityType();

        if (NEWER_OR_EQUALS_1_20_2) {
            return createModernSpawnPacket(type);
        }
        ...
        if (type == EntityTypes.EXPERIENCE_ORB) {
            ...
            return new WrapperPlayServerSpawnExperienceOrb(
                    ...
                    pos.getY(),
                    pos.getZ(),
                    this.entity.getMeta().getOptional(EntityMetaFields.ExperienceOrb.VALUE)
...
//                );
            ...
            if (type.isInstanceOf(EntityTypes.LIVINGENTITY)) {
        ...
        throw new IllegalStateException("Couldn't match a spawn packet for " + type + " on " + PacketEvents.getAPI().getServerManager().getVersion());
    }

    private WrapperPlayServerSpawnEntity createModernSpawnPacket(EntityType type) {
        Vector3d velocity = this.worldState.velocity();
